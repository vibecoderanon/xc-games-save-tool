...
"""
Final Xenoblade Chronicles (Wii) to Xenoblade Chronicles: Definitive Edition (Switch) Converter

Features:
1. Uses pre-clear reference save template (vibecoderanon - 2024.09.08 @ 00.35.38b4hx) so the game opens to the normal, pre-cleared title screen (ClearFlag = 0).
2. Updates Header Play Time at offset 0x04 for each slot so every load card displays the EXACT play time from the corresponding Wii save.
3. Updates Money (0x151B40), Active Party (0x152318), Character Levels, EXP, AP (0x152368), and Expert Mode settings.
4. Pairs each .sav file with its corresponding .tmb thumbnail file.

Output Mapping:
- monado01 -> bfsgame00.sav & bfsgame00.tmb (Manual Slot 1: Melia Lv 58, 127h 18m)
- monado02 -> bfsgame01.sav & bfsgame01.tmb (Manual Slot 2: Melia Lv 58-59, 127h 50m - Latest)
- monado03 -> bfsgame02.sav & bfsgame02.tmb (Manual Slot 3: Fiora Lv 55, 111h 45m)
- bfssystem.sav (Pre-clear system save)
"""

REF_PRE_CLEAR_DIR = r"c:\Users\teit\Documents\Antigravity\xc games save tool\vibecoderanon - 2024.09.08 @ 00.35.38b4hx"
...
def convert_slot(wii_filename, ref_sav_filename, ref_tmb_filename, out_sav_filename, out_tmb_filename):
    wii_path = os.path.join(WII_DIR, wii_filename)
    ref_sav_path = os.path.join(REF_PRE_CLEAR_DIR, ref_sav_filename)
    ref_tmb_path = os.path.join(REF_PRE_CLEAR_DIR, ref_tmb_filename)
    ...
    print(f" Template source: {ref_sav_filename}")
    print(f"========================================================")
    ...
    # 2. Read reference pre-clear Switch save template into bytearray
    with open(ref_sav_path, "rb") as f:
        sav = bytearray(f.read())

    # 3. Ensure ClearFlag = 0 at offset 0x4C (Normal Pre-Clear Title Screen)
    sav[0x4C:0x50] = struct.pack("<I", 0)

    # 4. Update Header Play Time at offset 0x04 (Little Endian uint32 seconds)
    sav[0x04:0x08] = struct.pack("<I", total_play_seconds)

    # 5. Update Money at 0x151B40 (Little Endian uint32)
...
def main():
    ...
    saves = [
        # (wii_file, ref_sav_template, ref_tmb_template, out_sav_name, out_tmb_name)
        ("monado01", "bfsgame00.sav", "bfsgame00.tmb", "bfsgame00.sav", "bfsgame00.tmb"), # Manual Slot 1
        ("monado02", "bfsgame01.sav", "bfsgame01.tmb", "bfsgame01.sav", "bfsgame01.tmb"), # Manual Slot 2 (Latest)
        ("monado03", "bfsgame02.sav", "bfsgame02.tmb", "bfsgame02.sav", "bfsgame02.tmb"), # Manual Slot 3
    ]

    for wii_file, ref_sav, ref_tmb, out_sav, out_tmb in saves:
        convert_slot(wii_file, ref_sav, ref_tmb, out_sav, out_tmb)

    # Copy pre-clear bfssystem.sav
    ref_system = os.path.join(REF_PRE_CLEAR_DIR, "bfssystem.sav")
